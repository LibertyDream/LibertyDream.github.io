...
\end{aligned}
$$
可以把既定策略看作是随机情况的特例，即概率分布中只有一个非零值，对应某个行动。实际上在 DPG 论文中，作者指出如果随机策略 $$\pi_{\mu_\theta, \sigma}$$ 是既定策略 $$\mu_\theta$$ 和变分变量 $$\sigma$$ 重新参数化了的，随机策略最终等价于 $$\sigma=0$$ 时的确定型情景。相较于既定策略，我们认为随机策略会需要更多样本，因为它统合了整个状态和行动空间的数据。
