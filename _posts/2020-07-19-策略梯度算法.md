...
试想你在马尔可夫链上的状态间游走，最终随着时间推移，你落在某一状态上的概率不再变化——这就是 $$\pi_\theta$$ 的静态概率。$$d^\pi(s) = \lim_{t \to \infty} P(s_t = s \vert s_0, \pi_\theta)$$ 是从 $$s_0$$ 起，按策略 $$\pi_\theta$$ 走 $$t$$ 步后 $$s_t=s$$ 的概率。实际上 PageRank 算法之所以有效的一大原因就是因为马尔可夫链存在静态分布，感兴趣的话可以看看这篇[文章](https://jeremykun.com/2015/04/06/markov-chain-monte-carlo-without-all-the-bullshit/)。

自然的，可以判断基于策略的方法在连续空间内更有用。因为有无尽的行动和状态有待估计价值，也因此基于价值的方法在连续空间内的计算开销高到天上去了。比如在[广义策略迭代](https://libertydream.github.io/2020/07/05/强化学习综述/#-策略迭代)中，改善策略时 $$\arg\max_{a \in \mathcal{A}} Q^\pi(s, a)$$ 需要遍历整个行动空间，常要经受[维度诅咒](https://en.wikipedia.org/wiki/Curse_of_dimensionality)的折磨。
...
一个广泛使用的 REINFORCE 变式是从收益 $$G_t$$ 中减去一个基准值，以此*降低梯度估计方差同时保证偏差不变*（如果可能的话我们总想这样做）。举例来讲，一个常用基准是从行动价值中减掉状态价值，如果打算这样做，就在梯度上升更新中用收益  $$A(s, a) = Q(s, a) - V(s)$$ 进行计算。这篇[文章](https://danieltakeshi.github.io/2017/03/28/going-deeper-into-reinforcement-learning-fundamentals-of-policy-gradients/)漂亮地解释了为什么基准会对降低方差有效，还不算一系列策略梯度基础。
...
A3C 中，在评论家学习价值函数的同时，多个演员在并行训练并且不时通过全局参数进行同步。因此，可以说 A3C 是为并行训练而生的。
...
A3C 能做到多代理训练的并行化。累计梯度那步（6.2）可以看作是最小批随机梯度更新的并行版：每个线程独立在各自方向上对 $$w$$ 或 $$\theta$$ 进行一点修正

#### A2C

[[论文](https://arxiv.org/abs/1602.01783)|[代码](https://github.com/openai/baselines/blob/master/baselines/a2c/a2c.py)]

**A2C** 是同步确定版的 A3C，这也是叫 “A2C” 的原因，第一个 “A” (“asynchronous”) 被删了。A3C 里每个代理人单独和全局参数沟通，所以有时线程参数会和不同版本的策略打交道，从而合在一起的更新效果并非最优。为解决这种不一致性，更新全局参数前 A2C 中的协调器会等所有演员的工作完成，然后下一轮迭代的时候演员们会从相同策略起步。同步梯度更新使训练更紧凑，无形中加快了收敛速度。

经[证实](https://blog.openai.com/baselines-acktr-a2c/)，A2C 在取得相同或超越 A3C 性能的同时，GPU 利用率更高，更适合大 batch 工作。
...
上面介绍的方法里，策略函数 $$\pi(. \vert s)$$ 总表示为当前状态下的行动 $$\mathcal{A}$$ 的概率分布，所以都是 *随机* 的。**既定策略梯度 （Deterministic policy gradient，DPG）** 则将策略作为确定的选择结果： $$a = \mu(s)$$。看上去可能有点怪——结果只有一个行动还怎么计算行动概率的梯度呢？来一步步的看一下。

便于讨论，变更一些符号含义：

- $$\rho_0(s)$$：状态的起始分布
- $$\rho^\mu(s \to s', k)$$：从状态 $$s$$ 开始，按策略 $$\mu$$ 走 $$k$$ 步后到达状态 $$s'$$ 的访问概率密度
- $$\rho^\mu(s')$$：打了折的状态分布<br/> $$\rho^\mu(s') = \int_\mathcal{S} \sum_{k=1}^\infty \gamma^{k-1} \rho_0(s) \rho^\mu(s \to s', k) \mathrm{d}s$$
...
\end{aligned}
$$
可以把既定策略看作是随机情况的特例，即概率分布中只有一个非零值，其对应某个行动。实际上在 DPG 论文中，作者指出如果随机策略 $$\pi_{\mu_\theta, \sigma}$$ 是被确定策略 $$\mu_\theta$$ 和变分变量 $$\sigma$$ 重新参数化了的，随机策略最终等价于 $$\sigma=0$$ 时的确定情况。相较于既定策略，我们认为随机策略会需要更多样本，因为它融合了整个状态和行动空间的数据。

既定策略梯度定理可以插到普通策略梯度框架当中。以策略性演员-评论家算法为例，算法每次迭代的时候两个行动选择确定性的 $$a = \mu_\theta(s)$$ ，而 [SARSA](https://libertydream.github.io/2020/07/05/强化学习综述/#-sarsa策略性-td-控制) 用上面刚算过的新梯度更新策略参数：
$$
\begin{aligned}
...
但除非环境内有足够的噪声，因为策略是确定的所以很难保证会进行足够的[探索](https://libertydream.github.io/2020/07/05/强化学习综述/#探索利用窘境)。可以选择往策略里加点噪声（可策略可就变成不确定的了！）或者按一个不同的随机行动策略采集点样本非策略地学习一下。
...
\end{aligned}
$$
因为策略是确定的，我们只需要 $$Q^\mu(s, \mu_\theta(s))$$ 而不是 $$\sum_a \pi(a \vert s) Q^\pi(s, a)$$ 作为给定状态 $$s$$ 的预计回报。用随机策略的非策略方法通常会用重要度采样修正行动和目标策略间的不协调，如前文所述。但既定策略梯度删掉了对行动的积分，也就不需要重要度采样了。

#### DDPG

[[论文](https://arxiv.org/pdf/1509.02971.pdf)|[代码](https://github.com/openai/baselines/tree/master/baselines/ddpg)]

**深度既定策略梯度（Deep Deterministic Policy Gradient）**，简写为 **DDPG**，是一个不建模非策略性的演员-评论家算法，结合了 [DPG](# DPG) 和 [DQN](https://libertydream.github.io/2020/07/05/强化学习综述/#-深度-q-网络)。记得吗，DQN 通过经历重放和冻结目标网络稳定了 Q 函数学习过程。原始 DQN 网络是用在离散空间上，DDPG 借演员-评论家框架将其拓展到连续空间，同时学习一个确定的策略。
...
论文中有个细节对机器人技术格外有帮助——怎样对低维特征的不同物理单元作归一化。例如，输入机器人的位置和速度让模型学习策略，这些物理统计信息天生不同，甚至同一类型的统计项在多个机器人间大相径庭。通过对最小批中样本进行[批归一化](http://proceedings.mlr.press/v37/ioffe15.pdf)规范各个维度，从而解决问题。
...
（1）**分布式评论家**：评论家将 Q 值的期望作为随机变量——一个经 $$w$$ 参数化的分布 $$Z_w$$，所以 $$Q_w(s, a) = \mathbb{E} Z_w(x, a)$$。学习分布参数的损失函数是要最小化两个分布间的距离——分布式 TD 误差：$$L(w) = \mathbb{E}[d(\mathcal{T}_{\mu_\theta}, Z_{w'}(s, a), Z_w(s, a)]$$，其中 $$\mathcal{T}_{\mu_\theta}$$ 是贝尔曼运算符。
...
\end{aligned}
$$
（2）**$$N$$ 步收益**：计算 TD 误差的时候，D4PG 会算 $$N$$ 步 TD 目标值而非一步，以此整合将来几步内的收益。所以新 TD 目标为：
$$
r(s_0, a_0) + \mathbb{E}[\sum_{n=1}^{N-1} r(s_n, a_n) + \gamma^N Q(s_N, \mu_\theta(s_N)) \vert s_0, a_0 ]
$$
（3）**多个分布式并行演员**：D4PG 用 $$K$$ 个独立演员并行收集经历，并将数据传到相同回放缓存区中
...
正式点的话，问题可以看作是多代理版的 MDP，也称为*马尔可夫游戏*。MADDPG 是为局部可观测的马尔可夫游戏提出的。比方说，这总共有 $$N$$ 个代理和一系列状态 $$\mathcal{S}$$，每个代理有一组可能的行动，$$\mathcal{A}_1, \dots, \mathcal{A}_N$$，以及一组观测 $$\mathcal{O}_1, \dots, \mathcal{O}_N$$。状态转移函数囊括整个状态、行动和观测空间 $$\mathcal{T}: \mathcal{S} \times \mathcal{A}_1 \times \dots \mathcal{A}_N \mapsto \mathcal{S}$$。各个代理的随机策略只包括它自己的状态和行动：$$\pi_{\theta_i}: \mathcal{O}_i \times \mathcal{A}_i \mapsto [0, 1]$$，这是个概率分布，考虑到自身观测或既定策略： $$\mu_{\theta_i}: \mathcal{O}_i \mapsto \mathcal{A}_i$$

令 $$\vec{o} = {o_1, \dots, o_N}$$, $$\vec{\mu} = {\mu_1, \dots, \mu_N}$$ 并用 $$\vec{\theta} = {\theta_1, \dots, \theta_N}$$ 对策略进行参数化

MADDPG 中的评论家要为 $$i$$ 号代理学习一个中心化的行动价值函数 $$Q^\vec{\mu}_i(\vec{o}, a_1, \dots, a_N)$$  ，其中 $$a_1 \in \mathcal{A}_1, \dots, a_N \in \mathcal{A}_N$$ 是所有代理的行动。对 $$i=1, \dots, N$$ 每个 $$Q^\vec{\mu}_i$$ 都独自进行学习，因而几个代理的回报结构可能五花八门，竞争环境中有了彼此冲突的回报。 与此同时，几个演员，各对应一个代理，也正在各自探索和更新策略参数 $$\theta_i$$。
...
$$
其中 $$\mathcal{D}$$ 是经历回放的记忆缓存区，其中包括多个事件样本 $$(\vec{o}, a_1, \dots, a_N, r_1, \dots, r_N, \vec{o}')$$——给定当前观测 $$\vec{o}$$，代理采取行动 $$a_1, \dots, a_N$$ 并获得回报 $$r_1, \dots, r_N$$，得到新观测 $$\vec{o}'$$
...
如果评论家更新期间策略 $$\vec{\mu}$$ 未知，可以让各代理学习并推演对其他代理策略的近似结果。就算用近似策略，MADDPG 仍能高效学习尽管推断出的策略可能不准。
...
为改善训练稳定性，应该避免参数一次对策略修改太多。**置信域策略优化（Trust region policy optimization，TRPO）**践行了这一想法，对每轮策略更新的幅度都加上了[ KL 散度](https://lilianweng.github.io/lil-log/2017/08/20/from-GAN-to-WGAN.html#kullbackleibler-and-jensenshannon-divergence)约束。

试想一个做非策略 RL 的场景，收集轨迹的策略 $$\beta$$ 和要优化的策略 $$\pi$$ 不同。非策略模型的目标函数是要计算状态造访分布和行动带来的总收益，而训练数据分布和真实策略状态分布间的错配由重要度采样估计函数进行弥补。
$$
\begin{aligned}
J(\theta)
...
按策略训练时，理论上收集数据的策略该和和要优化的策略一样才对，但当运算器和优化器并行异步运行的时候，行动策略可能会过期。TPRO 考虑到这点微小差异：将行动策略记为 $$\pi_{\theta_\text{old}}(a \vert s)$$ 所以目标函数变成：
$$
...
这样，在约束条件下新旧策略间的差异不会太大。同时 TRPO 仍能保证策略迭代时效果会不断改善（很妙，对吧？）感兴趣的话可以看看[论文](https://arxiv.org/pdf/1502.05477.pdf)中的证明

#### PPO

[[论文](https://arxiv.org/pdf/1707.06347.pdf)|[代码](https://github.com/openai/baselines/tree/master/baselines/ppo1)]

TRPO 还是有点复杂，但我们还想达到类似的约束效果，**近似策略优化（proximal policy optimization，PPO）**换成精简目标实现简化效果，同时保留了相似的性能。
...
$$\theta_\text{old}$$ 和 $$\theta$$ 之间没有限制，要最大化 $$J^\text{TRPO} (\theta)$$ 会因参数更新幅度和策略比例过大导致状态不稳定。PPO 通过强制 $$r(\theta)$$ 处于间隔在 1 左右的小区间内施加约束，准确地讲是 $$[1-\epsilon, 1+\epsilon]$$，其中 $$\epsilon$$ 是超参数。
$$
...
$$
函数 $$\text{clip}(r(\theta), 1 - \epsilon, 1 + \epsilon)$$ 会将比率限制在上到 $$1+\epsilon$$ 下至 $$1-\epsilon$$ 的范围内。PPO 的目标函数会选择原始值和剪切版中较小的一个，这样就不会为极致追求更好的回报而增大策略更新幅度了。

若策略（演员）和价值（评论家）函数都使用共享参数，在将 PPO 用到网络架构上的时候，除了修剪过的回报，目标函数也因价值估计（红色部分）上加了误差项而增强，因加了项信息熵（蓝色部分）使探索更充分。
$$
...
PPO 在一系列基准任务测试上证明虽经大幅简化，算法效果还是很好。

#### ACER

[[论文](https://arxiv.org/pdf/1611.01224.pdf)|[代码](https://github.com/openai/baselines/tree/master/baselines/acer)]

**经历重放演员-评论家算法（actor-critic with experience replay）**，简写为 **ACER** ，是一个带经历重放机制的非策略性演员-评论家模型，极大增强了采样效率并减少了数据勾连。A3C 给 ACER 打下了基础，但那是策略性的；ACER 算是非策略版 A3C。使A3C 摆脱策略的主要困难在于怎么控制非策略估计函数的稳定性。ACER 对此进行了三项设计：
...
回溯是种非策略、基于收益的 Q 值估计算法，对任何目标和行动策略对 $$(\pi, \beta)$$ 都保证能收敛，数据效力也同样不俗。

回想一下 TD 学习是怎么预测的：

1.  计算 TD 误差：$$\delta_t = R_t + \gamma \mathbb{E}_{a \sim \pi} Q(S_{t+1}, a) - Q(S_t, A_t)$$；其中 $$r_t + \gamma \mathbb{E}_{a \sim \pi} Q(s_{t+1}, a) $$ 就是 “TD 目标”。用期望 $$\mathbb{E}_{a \sim \pi}$$ 是因为后续几步我们能做出的最好估计是照当前策略 $$\pi$$ 走而获得的收益
2. 为达目的要修正误差更新值：$$Q(S_t, A_t) \leftarrow Q(S_t, A_t) + \alpha \delta_t$$。换句话说，Q 的增量与 TD 误差呈比例关系： $$\Delta Q(S_t, A_t) = \alpha \delta_t$$

如果是非策略性亮相，Q 值更新中需要做个重要度采样
$$
\Delta Q^\text{imp}(S_t, A_t) 
= \gamma^t \prod_{1 \leqslant \tau \leqslant t} \frac{\pi(A_\tau \vert S_\tau)}{\beta(A_\tau \vert S_\tau)} \delta_t
$$

重要度权重的乘积看起来非常吓人，不难想象它带来的方差有多大，甚至爆炸。回溯 Q 值估计方法修改了 $$\Delta Q$$，令权重不能超过常量 $$c$$ 从而简化了重要度权重计算：
$$
...
为减少策略梯度 $$\hat{g}$$ 的方差，ACER 以常量 $$c$$ 截断了重要度权重，加了个修正项。$$\hat{g}_t^\text{acer}$$ 是 $$t$$ 时刻的 ACER 策略梯度。
$$
\begin{aligned}
\hat{g}_t^\text{acer}
...
\end{aligned}
$$
其中 $$Q_w(.)$$ 和 $$V_w(.)$$ 是参数为 $$w$$ 的评论家预测的价值函数。第一项（蓝色）带有裁剪过的重要度权重。除了减掉当基准的状态价值函数 $$V_w(.)$$ ，修剪同样利于减少方差。第二项（红色）是为实现无偏估计而做的修正。

**高效 TRPO**

此外，ACER 还借鉴了 TRPO 的思想，只是做了点小改动使计算更高效：相较于计算更新前后两策略间的 KL 散度，ACER 维护了一个过往策略的动态平均值，并迫使更新后的策略不会偏离均值太远。

ACER 的论文充满了各种方程，所幸有了关于 TD 学习，Q 学习，重要度采用和 TRPO  的知识铺垫，你会发现论文也没那么难懂了。
...
> 首先考虑使新旧网络保持恒定 KL 散度的所有参数组合，该常量可以看作是步长或学习率。在所有可能组合中选择能最小化损失函数的那个

这里列出 ACTKR 主要是保证文章完整性，这里不会深究其中细节，因为涉及到大量有关自然梯度和优化论的理论知识。如果感兴趣的话，读 ACKTR 论文前先看看这些论文/文章：
...
> 近似分两步。首先将费希尔矩阵的行列分组，每部分都对应一个给定层的所有权重，同时也使矩阵区块化。之后将这些块作为对更小矩阵间克罗内克积的近似，这等价于基于网络梯度的统计信息而作的某些近似假设

> 第二步，矩阵进一步近似为可逆的对角或三角矩阵。通过仔细检查逆协方差，树结构图模型和线性回归间的关系，我们证实该近似可靠。尤其是该证明并不针对费希尔矩阵，我们的实验证明虽然逆费希尔矩阵确实有这种结构（近似），费希尔本身却并没有

#### SAC

[[论文](https://arxiv.org/abs/1801.01290)|[代码](https://github.com/haarnoja/sac)]

**柔性演员-评论家（Soft Actor-Critic，SAC）**算法为鼓励探索把策略的熵加到了回报里：我们希望能学到一种策略，行动尽可能随机同时仍能完成任务。这是个遵从最大熵强化学习框架的非策略演员-评论家模型。前辈工作是[柔性 Q 学习](https://arxiv.org/abs/1702.08165)。

SAC 的关键部分有三：

- 一个有独立策略和价值函数网络的[演员-评论家](# 演员-评论家)框架
- 一个[非策略](# 非策略策略梯度)性公式，为了效率要能复用早先收集到的数据
- 熵最大化保证稳定度和探索度
...
$$\rho_\pi(s)$$ 和 $$\rho_\pi(s, a)$$ 表示在策略 $$\pi(a \vert s)$$ 指导下，状态分布的状态边际和状态-行动边际；[DPG](# DPG) 部分有相似定义。
...
\end{aligned}
$$
其中 $$\bar{\psi}$$ 是目标价值函数，函数是指数变化的均值（或只是定期“硬性”更新），就像 DQN 里为稳定训练而对目标 Q 网络的参数所作的那样。
...
\end{aligned}
$$
其中 $$\Pi$$ 是一系列可对策略建模的潜在策略，要保证他们好处理；比如，$$\Pi$$ 可以是高斯混合分布家族，建模成本高但表达能力强而且好处理。$$Z^{\pi_\text{old}}(s_t)$$ 是配分函数，负责对分布归一化，通常比较棘手但对梯度没影响。怎么最小化 $$J_\pi(\theta)$$ 取决于对 $$\Pi$$ 的选择。

这一更新过程保证 $$Q^{\pi_\text{new}}(s_t, a_t) \geqslant Q^{\pi_\text{old}}(s_t, a_t)$$，该引理的证明见原[论文](https://arxiv.org/abs/1801.01290)附录 B.2。
...
就温度参数来讲 SAC 很脆弱，不幸的是它又很难调整温度。因为随着策略变得更好，无论是任务期间还是训练过程中，熵的变化都可能难以捉摸。一个对 SAC 的改良是构建一个有约束优化问题：最大化期望收益的同时，策略要满足一个最小熵约束：
$$
...
预期收益 $$\mathbb{E} \Big[ \sum_{t=0}^T r(s_t, a_t)\Big]$$ 可以分解成各时刻回报的和。因为 $$t$$ 时刻的策略 $$\pi_t$$ 对之前的策略 $$\pi_{t-1}$$ 没影响，我们可以倒退令各步收益最大——这实际上是**动态规划（DP）**。
$$
...
$$
要解带不等式约束的求最大的最优化问题，可以用拉格朗日乘子（也叫“对偶变量”）构建[拉格朗日表达式](https://cs.stanford.edu/people/davidknowles/lagrangian_duality.pdf)，$$\alpha_T$$:
$$
L(\pi_T, \alpha_T) = f(\pi_T) + \alpha_T h(\pi_T)
$$
试想一个*就 $$\alpha_T$$ 最小化 $$L(\pi_T, \alpha_T)$$* 的场景——给定特定值 $$\pi_T$$
...
$$
所以为了最大化 $$f(\pi_T)$$，对偶问题列在下面了。注意为保证 $$\max_{\pi_T} f(\pi_T)$$ 正好最大化而且不会变成 $$-\infty$$，必须满足约束条件。
$$
\begin{aligned}
...
\end{aligned}
$$
可以交替计算最优的 $$\pi_T$$ 和 $$\alpha_T$$。首先给定当前 $$\alpha_T$$，得到最佳策略 $$\pi_T^{*}$$，使表达式最大 $$L(\pi_T^{*}, \alpha_T)$$。然后带入 $$\pi_T^{*}$$ 并计算 $$\alpha_T^{*}$$ 最小化 $$L(\pi_T^{*}, \alpha_T)$$。假设我们对策略有一个神经网络，对温度参数也有一个，交替更新过程更要看我们训练的时候怎么更新网络参数。
$$
\begin{aligned}
\pi^{*}_T
...
&= \min_{\alpha_{T-1} \geqslant 0}  \max_{\pi_{T-1}} \Big( Q^{*}_{T-1}(s_{T-1}, a_{T-1}) - \alpha^{*}_T \mathcal{H}(\pi^{*}_T) + \alpha_{T-1} \big( \mathcal{H}(\pi_{T-1}) - \mathcal{H}_0 \big) \Big) & \text{; dual problem w/ Lagrangian.} \\
...
更新<span style="color: #32CD32;">绿色</span> $$\alpha_{T-1}$$ 的方程和更新上面<span style="color: #1E90FF;">蓝色</span> $$\alpha_{T-1}$$ 的方程形式相同。不断重复这一过程，通过最小化相同的目标函数 $$J(\alpha)$$ 可以学到每一步的最佳温度参数：
$$
...
*图 7  温度自动调节的柔性演员-评论家算法*

#### TD3

[[论文](https://arxiv.org/abs/1802.09477)|[代码](https://github.com/sfujim/TD3)]

人们都知道 Q 学习算法有高估价值函数的问题，过高的估计会在迭代训练中传播开来并对策略造成负面影响。这也直接推动了[双 Q 学习](https://papers.nips.cc/paper/3964-double-q-learning)和[双 DQN](https://arxiv.org/abs/1509.06461) 的出现：通过用两个价值网络对行动选择和 Q 值更新进行解耦。

**孪生延迟深度决策（Twin Delayed Deep Deterministic，TD3）**在 DDPG 之上做了点改动来防止价值函数的高估：

（1）**精简双 Q 学习**：双 Q 学习中，行动选择和 Q 值估计分别由两个网络完成。DDPG 中，给定两个既定演员 $$(\mu_{\theta_1}, \mu_{\theta_2})$$ 和两个相应的评论家 $$(Q_{w_1}, Q_{w_2})$$，双 Q 学习的贝尔曼目标看起来是这样的：
$$
\begin{aligned}
...
但因为慢变策略，这两个网络可能太像了以至于不能独立进行决策。相反，*精简双 Q 学习* 使用两估计中的最小值从而低估偏差，这很难通过训练传播：
$$
\begin{aligned}
...
（2）**延迟更新目标和策略网络**：演员-评论家模型中，策略和价值更新是深度绑定的：策略很差的时候，过高的估计使所估价值各不相同，同时如果价值估计本身不准策略也会变差。

为减少方差，TD3 更新策略的频率比 Q 函数要稍低些。直到几轮更新后价值误差已足够小，策略网络才会进行更新否则保持不变。这和 [DQN](https://libertydream.github.io/2020/07/05/强化学习综述/#-深度-q-网络) 里定期更新的目标网络作为稳定目标的做法类似。
...
\end{aligned}
$$
该做法借鉴了 [SARSA](https://libertydream.github.io/2020/07/05/强化学习综述/#-sarsa策略性-td-控制) 的更新思想，更加认定相似行动应该有相似价值。最终算法如下：
...
最大熵策略优化中，$$\theta$$ 被看作是一个随机变量 $$\theta \sim q(\theta)$$，并且希望模型能学到分布 $$q(\theta)$$。假设我们能先验的知道 $$q$$ 的大概情况，$$q_0$$，同时希望能通过优化下列目标函数，指导学习过程别让 $$\theta$$ 偏离 $$q_0$$ 太远：
$$
...
如果没有任何先验信息，可以令 $$q_0$$ 为均匀分布，$$q_0(\theta)$$ 为常量。然后上述目标函数就变成 [SAC](# SAC) 了，其中信息熵的那项会推动探索：
$$
\begin{aligned}
\hat{J}(\theta) 
...
\end{aligned}
$$
针对 $$q$$ 对 $$\hat{J}(\theta) = \mathbb{E}_{\theta \sim q} [J(\theta)] - \alpha D_\text{KL}(q\|q_0)$$ 求导：
$$
\begin{aligned}
\nabla_q \hat{J}(\theta) 
...
在用 SVGD 法估计目标后验分布 $$q(\theta)$$ 的时候，要依赖于一系列算子 $$\{\theta_i\}_{i=1}^n$$ （独立训练的策略代理人）同时每个算子按如下方式更新：
$$
...
$$
其中 $$\epsilon$$ 是学习率，$$\phi^{*}$$ 是 $$\theta$$  状价值向量的一个 RKHS（再生希尔伯特空间）$$\mathcal{H}$$ 的单位球面，会最大程度减少算子与目标分布间的 KL 散度值。$$q'(.)$$ 是 $$\theta + \epsilon \phi(\theta)$$ 的分布。
...
$$\phi^{*}$$ 估计的形式如下，正定核 $$k(\vartheta, \theta)$$，即高斯[径向基函数](https://en.wikipedia.org/wiki/Radial_basis_function)，负责衡量两算子间的相似度。
$$
\begin{aligned}
\phi^{*}(\theta_i) 
&= \mathbb{E}_{\vartheta \sim q'} [\nabla_\vartheta \log q(\vartheta) k(\vartheta, \theta_i) + \nabla_\vartheta k(\vartheta, \theta_i)]\\
&= \frac{1}{n} \sum_{j=1}^n [\color{red}{\nabla_{\theta_j} \log q(\theta_j) k(\theta_j, \theta_i)} + \color{green}{\nabla_{\theta_j} k(\theta_j, \theta_i)}] & \scriptstyle{\text{； 以当前算子值近似 }q'}
\end{aligned}
$$

-  <span style="color:#fc0303;">红色</span>的第一项推动 $$\theta_i$$ 向着 $$q$$ 的高概率区间学习，那里共享着相似的算子。=> 尽量与其他算子相似
-   <span style="color:#00c925;">绿色</span>的第二项会将算子彼此推离从而使策略多样化。=> 尽量和其他算子保持差异

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-09-07_SVPG.png)

一般温度 $$\alpha$$ 会采用模拟退火方案，训练初期多探索，后期多利用。

#### IMPALA

[[论文](https://arxiv.org/abs/1802.01561)|[代码](https://github.com/deepmind/scalable_agent)]

为了增强 RL 获得更强生产力，**IMPALA**（重要度加权演员-学习者框架，“Importance Weighted Actor-Learner Architecture”）在基础的演员-评论家配置之上将行动从学习中解耦，并通过 **V-trace** 非策略修正从所有经历轨迹中学习。

几个演员并行生成经历，而学习者会用这些经历优化策略和价值函数参数。演员会定期从学习者那里拿最新策略更新自己的参数。因为行动和学习解耦了，每个时间单元内我们可以加入更多演员生成更多轨迹。因为训练策略和行动策略不是完全同步的，两者之间有点差距所以需要非策略性的修正。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-09-07_IMPALA.png)

令价值函数 $$V_\theta$$ 参数为 $$\theta$$，策略 $$\pi_\phi$$ 参数为 $$\phi$$。还知道回放缓存中的轨迹是由稍旧些的策略 $$\mu$$ 收集得来的。

在训练中的 $$t$$ 时刻，给定 $$(s_t, a_t, s_{t+1}, r_t)$$，价值函数参数 $$\theta$$ 通过当前价值与 V-trace 目标价值间的 L2 损失进行学习。$$n$$ 步 V-trace 目标为：
$$
\begin{aligned}
...
\end{aligned}
$$
<span style="color:#fc0303;">红色</span>部分 $$\delta_i V$$ 是 $$V$$ 的时差。 $$\rho_i = \min\big(\bar{\rho}, \frac{\pi(a_i \vert s_i)}{\mu(a_i \vert s_i)}\big)$$ 和 $$c_j = \min\big(\bar{c}, \frac{\pi(a_j \vert s_j)}{\mu(a_j \vert s_j)}\big)$$ 是*精简化[重要度采样（IS）](# 非策略策略梯度)权重*。$$c_t, \dots, c_{i-1}$$ 的积表示 $$i$$ 时刻观察到的时差 $$\delta_i V$$ 对前一时刻 $$t$$ 时价值函数更新的影响大小。策略性场景下，有 $$\rho_i=1$$ 和 $$c_j=1$$（假设 $$\bar{c} \geqslant 1$$），因此 V-trace 目标变成了策略性 n 步贝尔曼目标。

$$\bar{\rho}$$ 和 $$\bar{c}$$ 是两个修剪常量， $$\bar{\rho} \geqslant \bar{c}$$。$$\bar{\rho}$$ 影响价值函数收敛定点， $$\bar{c}$$ 影响收敛速度。当 $$\bar{\rho} =\infty$$（不修剪），会收敛于目标策略 $$V^\pi$$ 的价值函数；当 $$\bar{\rho} \rightarrow 0$$，计算行动策略 $$V^\mu$$ 的价值函数；处于两者之间时，计算 $$\pi$$ 和 $$\mu$$ 之间的策略。
...
\end{aligned}
$$
其中 $$r_t + \gamma v_{t+1}$$ 是 Q 值的估计，要从中减掉状态基准 $$V_\theta(s_t)$$。$$H(\pi_\phi)$$ 是为鼓励探索加上的熵。

实验中，IMPALA 用来在多任务上训练代理。涉及两个不同的模型框架，一个浅模型（左）和一个深度残差模型（右）。
...
- 努力降低方差并保证偏差不变使训练趋于稳定
- 非策略性带来更好的探索效果，有利于更充分的利用数据样本
- 经历重放（训练数据从记忆回放缓存中采样）
- 目标网络要么定期冻结，要么比积极学习的策略网络更新的更慢些
- 批归一化
- 熵正则化奖励
- 评论家和演员共享网络低层参数，两个结果指向策略和价值函数
- 可以用既定策略学习而不是随机策略
- 对策略更新间的差异加上约束
- 新优化方法（比如 K-FAC）
- 策略的熵最大化利于推动探索
