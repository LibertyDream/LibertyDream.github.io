...
[Hse 等，2020](https://arxiv.org/abs/2009.10897) 重新审视了 PPO 中的两个设计，简单讲 (1) 用受限概率比率做策略正则化，（2）通过连续高斯或离散 softmax 分布将策略行动空间参数化。作者先是确认了三个失败的 PPO 形式，然后提出了这两个设计的替代方案。

失败形式是：

1. 连续行动空间中，奖励在有限支持范围内消失时标准 PPO 就不稳定
2. 零星高回报的离散行动空间中，标准 PPO 常陷入次优行动不能自拔
3. 当有局部最优行动离初始值很近时，策略对初始化很敏感

结合高斯策略，将行动空间离散化或者用 Beta 分布可以避免败状 1 和 3。用 KL 正则化（和 TRPO 思路一样）作备选代理模型可以解决败状 1 和 2。


$$
\begin{aligned}
\mathcal{L}^{\text {CLIP }}(\theta) &:=\underset{a, s \sim \pi_{\text {old }}}{\mathbb{E}}\left[\min \left(\frac{\pi_{\theta}(a \mid s)}{\pi_{\text {old }}(a \mid s)} \hat{A}^{\pi_{\text {old }}}(a, s), \operatorname{clip}\left(\frac{\pi_{\theta}(a \mid s)}{\pi_{\text {old }}(a \mid s)}, 1-\epsilon, 1+\epsilon\right) \hat{A}^{\pi} \text { old }(a, s)\right)\right] \\
\mathcal{L}^{\text {KL,forward }}(\theta) &:=\underset{a, s \sim \pi_{\text {old }}}{\mathbb{E}}\left[\frac{\pi_{\theta}(a \mid s)}{\pi_{\text {old }}(a \mid s)} \hat{A}^{\pi_{\text {old }}(a, s)}\right]-\beta D_{K L}\left(\pi_{\text {old }} \| \pi_{\theta}\right) \\
\mathcal{L}^{\text {KL,reverse }}(\theta) &:=\underset{a, s \sim \pi_{\text {old }}}{\mathbb{E}}\left[\frac{\pi_{\theta}(a \mid s)}{\pi_{\text {old }}(a \mid s)} \hat{A}^{\pi_{\text {old }}}(a, s)\right]-\beta D_{\mathrm{KL}}\left(\pi_{\theta} \| \pi_{\text {old }}\right)
\end{aligned}
$$
...
策略和价值网络间共享参数有利有弊。一方面可以让策略和价值函数共享彼此所学到的特征，但另一方面这也可能导致目标间的对抗，还要求要在同一时刻用相同数据训练两个网络。**阶段式策略梯度**（**Phasic policy gradient, PPG**; [Cobbe 等人，2020](https://arxiv.org/abs/2009.04416)）修改了传统策略性演员-评论家策略梯度算法，准确讲是 PPO，希望策略和价值函数有独立的训练阶段。两个阶段交替：

1. *策略阶段*：通过优化 PPO 目标 $$L^{\mathrm{CLIP}}(\theta)$$ 更新策略网络
2. *辅助阶段*：优化行为克隆损失的同时优化一个辅助目标。论文中，价值函数误差是唯一的辅助目标，但它可以很通用，囊括了其他任意辅助损。


$$
\begin{aligned}
L^{\text {joint }} &=L^{\text {aux }}+\beta_{\text {clone }} \cdot \mathbb{E}_{t}\left[\operatorname{KL}\left[\pi_{\theta_{\text {old }}}\left(\cdot \mid s_{t}\right), \pi_{\theta}\left(\cdot \mid s_{t}\right)\right]\right] \\
L^{\text {aux }} &=L^{\text {value }}=\mathbb{E}_{t}\left[\frac{1}{2}\left(V_{w}\left(s_{t}\right)-\hat{V}_{t}^{\text {targ }}\right)^{2}\right]
\end{aligned}
$$


其中 $$\beta_{\text {clone }}$$ 是控制分歧程度的超参数，决定了优化辅助目标的同时，希望策略不偏离其原行为的程度

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2021-01-16_PPG_algo.png)

*图 6  PPG 算法*

其中

- $$N_{\pi}$$ 是策略阶段策略更新迭代的次数。注意策略阶段在每个辅助阶段做了多次更新迭代。
- $$E_{\pi}$$ 和 $$E_{V}$$ 分别控制着策略和价值函数的样本复用。注意这发生在策略阶段内部，所以 $$E_{V}$$ 影响到的是实际价值函数的学习而非辅助价值函数。
- $$E_{\mathrm{aux}}$$ 控制了辅助阶段的样本复用。PPG 中，价值函数的优化可以容纳层次高得多的样本复用水平，比如论文中的实验，$$E_{\text {aux }}=6$$ 而 $$E_{\pi}=E_{V}=1$$。

相较于 PPO，PPG 显著改善了样本效率

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2021-01-16_PPG_exp.png)

*图 7  在 Procgen 基准上 PPG 和 PPO 的平均归一化后的性能*
