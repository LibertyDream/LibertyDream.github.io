...
- 最小化神经网络的高度非凸误差函数的时候，常见的另一个重大挑战是避免陷入很多并非最优的局部最小值中。[Dauphin 等人](https://arxiv.org/abs/1406.2572)指出带来难度的实际不是局部最小值，而是鞍点，也就是那些一面斜率增大另一边却下降的点。这些鞍点一般周围都是误差相同的高原，使得 SGD 想要逃离异常困难，因为各个维度上的梯度都近乎为 0。
...
$$\gamma v_{t-1}$$ $$\theta$$  $$\theta-\gamma v_{t-1}$$ $$\theta$$
$$
\begin{aligned}
...
$$\theta$$ $$\theta_{i}$$  $$\eta$$ $$\theta_{i}$$ $$g_{t}$$ $$g_{t,i}$$ $$\theta_{i}$$
$$
...
$$R M S[\Delta \theta]_{t}$$ $$R M S[\Delta \theta]_{t-1}$$
$$
\begin{aligned}
...
$$
$$\eta=0.002$$，$$\beta_{1}=0.9$$  $$\beta_{2}=0.999$$
