...
tags:
    - 
excerpt:    梯度下降一般是优化神经网络和其他机器学习算法的首选方式，但常被当作一个黑箱来用。本文仔细研究了许多流行梯度优化算法的实际工作模式，比如 Momentum，Adagrad，Adam 等，补充了相关算法的最新进展以及算法外的附加策略。
---
...
最后，我们介绍几个额外策略，可以和上面任意一个算法搭配使用，进一步改善 SGD 性能。其他的一般性技巧概要可以看 [LeCun 等，1998](https://doi.org/10.1007/3-540-49430-8_2)

#### 混淆与课程学习

一般来说，我们希望避免将样本以带有某种意义的顺序传入模型，这可能导致带偏优化算法。所以每轮训练后混淆打乱一下训练数据成为了一种常见操作。

可另一方面，有些场合我们要解决的是越来越难的问题，这时把训练样本以有意义的顺序传入实际有可能会提高模型性能，收敛效果更好。构建这种有意义顺序的方法叫做课程学习（[Bengio 等，2009](https://doi.org/10.1145/1553374.1553380)）。

[Zaremba 和 Sutskever，2014](https://arxiv.org/abs/1410.4615) 只是用课程学习方法训练了 LSTM，评估了些简单程序，实验将样本按难度升序排列，结果显示组合或混合策略效果要比对照组好。

#### 批归一化

为方便学习，一般我们会用 0 均值和单位方差对参数的初始值做归一化。随着训练的进行，参数会更新到不同幅度，开始的归一化效果消失，这导致训练速度下降，并且随着网络变得越来越深，变化与差异会被进一步放大。

[Ioffe 和 Szegedy，2015](https://arxiv.org/pdf/1502.03167) 提出了批归一化方法，对每个最小批重新进行归一化，同时凭此将变动反向传播出去。给模型架构加上归一化部分后，可以尝试更大的学习率，也不用那么太在意参数初始值了。此外，批归一化还有部分正则化的效用，降低了（有时甚至会消除）对 Dropout 的需求。
...
作者证明加上这种噪声能使网络对糟糕的初始值更鲁棒，能帮着训练特定又深又复杂的网络。他们猜加上的噪声让模型更有可能逃离并找到新的局部最优，模型越深几率越高。
