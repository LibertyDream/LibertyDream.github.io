...
要用梯度下降更新结构搜索模型，需要使选择离散运算的过程可微。这些方法一般会把结构参数和网络权重的学习打包到一块扔到模型里。更多内容见下面[“单样本”方法](# 单样本法：搜索+评估)一节
...
*图 14  高效架构搜索（EAS）基于 RL 的元控制器一览。对架构配置编码后，其会通过两个独立演员网络输出 net2net 转换行为（图片来源：[Cai 等人，2017](https://arxiv.org/abs/1707.04873)）*

受到类似的启发，高效 NAS（**ENAS**；[Pham 等人，2018](https://arxiv.org/abs/1802.03268)）通过在子模型间大量共享参数对 NAS 进行加速（小 1000 倍）。ENAS 核心洞见是它观察到所有采样得到的结构图都能看作是一张大*超图（supergraph）*的*子图（sub-graph）*。所有子网络都在共享该超图的权重。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_ENAS-example.png)

*图 15  （左）4 结点循环单元构成的完整搜索空间，但只有红色连接被激活（中）举例说明左侧激活子图怎么变成子模型结构（右）RNN 控制器给中间架构生成的网络参数（图片来源：[Pham 等人，2018](https://arxiv.org/abs/1802.03268)）*
...
常规子模型的评价套路是用标准梯度下降更新模型权重。SMASH ([Brock 等人，2017](https://arxiv.org/abs/1708.05344)) 有个不同的有趣想法：*能不能直接靠网络结构参数预测模型权重？*

他们结合模型结构配置的编码信息用 [HyperNet](https://blog.otoro.net/2016/09/28/hyper-networks/) ([Ha 等人，2016](https://arxiv.org/abs/1609.09106)) 直接生成模型权重，然后直接带着 HyperNet 给的参数去验证模型。注意我们不必对每个子模型额外做训练，但得训练 HyperNet。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_SMASH-algorithm.png)

*图 16  SMASH 算法（图片来源：[Brock 等人，2017](https://arxiv.org/abs/1708.05344)）*
...
*图 17  SMASH 误差相关性（图片来源：[Brock 等人，2017](https://arxiv.org/abs/1708.05344)）*

SMASH 可以看作是实现参数共享的另一种方式。[Pham 等人，2018](https://arxiv.org/abs/1802.03268) 指出 SMASH 有个问题是：因为权重是张量求积得到的，HyperNet 的使用限制了 SMASH 子模型在*低阶空间（low-rank space）*的权重。相对而言，ENAS 就没这个限制。

### 单样本法：搜索+评估

为一众子模型分别进行搜索和评估代价太高，我们已经见识了像 [Brock 等人，2017](https://arxiv.org/abs/1708.05344) 或 [Pham 等人，2018](https://arxiv.org/abs/1802.03268) 提出的方法，单独训练一个模型就足以评估搜索空间中的任意子模型。

**单样本（one-shot）**结构搜索进一步发展了共享权重的思想，并将结构生成与权重参数结合的更加紧密。下列方法全都将子结构的视作某张超图的不同子图，超图里公共边的权重共享。

[Bender 等人，2018](http://proceedings.mlr.press/v80/bender18a/bender18a.pdf) 构建了一个很大的过参数化网络，称为**单样本模型（One-Shot model）**，它包括搜索空间内所有可能的操作。借助 ScheduledDropPath（丢弃率随时间增长，训练终了达到 $$r^{1/k}$$，其中 $$0 < r < 1$$ 是个超参数而 $$k$$ 是抵达路径数）和一些精巧设计（比如 ghost 批归一化，只对激活结构做 L2 正则化），这么大一个模型的训练会很稳定，可用于评估超图采样得到的任何子模型。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_one-shot-model-architecture.png)

*图 18  单样本模型架构。每个单元有 $$N$$ 个选择块，每个选择块最多选 2 个运算。实线边每个结构都用，虚线是可选边（图片来源：[Bender 等人，2018](http://proceedings.mlr.press/v80/bender18a/bender18a.pdf)）*

一旦单样本模型训练好了，通过清零或删掉些边随机采样些不同结构出来，喂给它进行性能评估。采样过程可以用 RL 或演化代替。

作者们观察到用单样本模型测得的准确率和同架构微调后的准确率差异可能会非常大。他们猜单样本模型会自动学会关注网络里 *最有用* 的操作并且变得只要操作可用就很 *依赖* 这些操作。所以清空有用操作会大幅降低模型准确率，而删掉些无关紧要的部分只会带来很小的影响——所以在用单样本模型评估的时候我们会看到更大的得分方差。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_one-shot-model-accuracy-correlation.png)

*图 19  模型分级样例，不同的单样本模型准确率与他们作为独立模型时的真实验证准确率（图片来源：[Bender 等人，2018](http://proceedings.mlr.press/v80/bender18a/bender18a.pdf)）*

很明显设计这样一个搜索图不是个普通活，但它确实展现了单样本法的巨大潜力。只用梯度下降它的效果就很好，不需要像 RL 和 EA 这样的额外算法。

有些人认为 NAS 低效的一个主要原因是将结构搜索看成是一个 *黑箱最优化* 问题，所以我们会陷入像 RL，演化，SMBO等方法泥潭之中。如果转而依靠标准梯度下降，有可能使搜索过程更高效。于是乎， [Liu 等人，2019](https://arxiv.org/abs/1806.09055) 提出了微分结构搜索（Differentiable Architecture Search，**DARTS**）。DARTS 对检索超图中的每条路都加上了了一个连续松弛操作，从而能通过梯度下降一并训练结构参数与权重。
...
*图 20  DARTS 对 DAG 超图中的边进行连续松弛并确认最终模型（图片来源： [Liu 等人，2019](https://arxiv.org/abs/1806.09055)）*
...
*图 21  DARTS 算法概览（图片来源： [Liu 等人，2019](https://arxiv.org/abs/1806.09055)）*

还有一个想法和 DARTS 类似的，随机 NAS（Stochastic NAS；[Xie 等人，2019](https://arxiv.org/abs/1812.09926)）用具体的分布（具体是指离散随机变量的连续松弛；[Maddison 等人，2017](https://arxiv.org/abs/1611.00712)）和再参数化技巧进行连续松弛，它的目标和 DARTS 相同，都是使离散分布可微从而可以用梯度下降做最优化。

DARTS 能大幅减少 GPU 耗时，作者做了 CNN 单元搜索实验，其中 $$N=7$$，用一块 GPU 只花了 1.5 天。但因为网络结构的连续表示，它也有高 GPU 内存消耗的问题，为了将模型塞入单 GPU 的内存中，作者选了个小 $$N$$。

为了限制 GPU 内存开销，**ProxylessNAS** ([Cai 等人，2019](https://arxiv.org/abs/1812.00332)) 将 NAS 看作是在 DAG 路径上的剪枝过程，并使结构参数二值化，迫使每次在两结点间只激活一条路。边被遮蔽与否的概率会稍后通过少数二值结构采样进行学习，并用 *BinaryConnect* ([Courbariaux 等，2015](https://arxiv.org/abs/1511.00363)) 更新相应概率。 ProxylessNAS 表明 NAS 和模型压缩间关系密切，通过路径压缩它能将内存开销降低一个数量级。

接着回到图表示。在 DAG 邻接矩阵 $$G$$ 中，$$G_{ij}$$ 是结点 $$i$$ 和 $$j$$ 间的边，其值可从基本操作集 $$\vert \mathcal{O} \vert$$ 里挑，$$\mathcal{O} = \{ o_1, \dots \}$$。无论是单样本模型还是 DARTS 和 ProxylessNAS 都把各边看作是操作的混合体 $$m_\mathcal{O}(x)$$，但略有不同。

单样本模型中 $$m_\mathcal{O}(x)$$ 就是所有操作的和。DARTS 里则是加权和，权重通过对长为 $$\vert \mathcal{O} \vert$$ 的实值架构权重向量 $$\alpha$$ 求 softmax 得到。ProxylessNAS 把 $$\alpha$$ 的 softmax 概率变成二值门，并用二值门保证每次只激活一个操作。
