...
尽管多数成功且流行的模型架构是由人类专家设计的，但不意味着我们已经探索完了整个网络结构空间并找到了最佳选择。如果以一种系统且自动的方式学习高性能模型架构，找到最佳结果的胜算会更大些。
...
受视觉模型架构（如 Inception，ResNet）的成功启发，特别是复用组件这一设计思路影响，*NASNet 搜索空间*（[Zoph 等，2018](https://arxiv.org/abs/1707.07012)）设计了一种卷积网络结构，网络就是相同单元的多次重复，并且每个单元里面都有几个 NAS 算法预测的操作。精心设计的单元组件可在数据集间的迁移。通过调整单元重复次数也很容易减少或增大模型规模。
...
为了利用已发现的精心设计过的网络[样式](https://en.wikipedia.org/wiki/Network_motif)，NAS 搜索空间可以转为分级结构，就像在*分级 NAS（Hierarchical NAS，HNAS）*（[Liu 等，2017](https://arxiv.org/abs/1711.00436)） 中一样。起初是一小撮初始运算，包括卷积，池化，单位阵等在内的单独操作，然后由初始运算构成的小子图（或“样式”）递归地构建上层计算图。
...
*图 5  （上）三个 1 级初始运算构成一个 2 级样式（下）将三个 2 级样式插入基础网络集成为一个 3 级样式（图片来源：[Liu 等，2017](https://arxiv.org/abs/1711.00436)）*
...
和 NASNet 一样，[Liu 等，2017](https://arxiv.org/abs/1711.00436)的实验目的在于在预先定义好的带重复组件的”宏“结构中找到较好的单元结构，实验显示就算用简单的搜索方法（比如随机搜索或演化算法），精心设计过的搜索空间也能显著增强其性能。

[Cai 等，2018b](https://arxiv.org/abs/1806.02639)提出了一种树结构的搜索空间，用上了路径层网络转换。树中每个结点都设有给子结点划拨输入的 *分配（allocation）* 计划，和组合子结点结果的 *整合（merge）* 计划。如果某一层加了或连了整合计划，路径层网络转换能将其划成一个多叉样式。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_path-level-network-transformations.png)

*图 6  用路径层转换将单一层转换为树结构样式（图片来源：[Cai 等，2018b](https://arxiv.org/abs/1806.02639)）*

#### 记忆库表示

前馈网络的记忆库表示方法是[Brock 等，2017](https://arxiv.org/abs/1708.05344) 在 [SMASH](# 从预测出发) 中提出的。相较于运算图，作者将神经网络视作有多个可读写的记忆块的系统。每层操作被设计为：（1）从记忆块子集中读取；（2）计算结果；（3）将结果写入另一子集。比如在序列模型中，会不断从单个记忆块中读取与覆盖。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_NAS-memory-bank-view-representation.png)

*图 7  几个流行网络架构块的记忆库表示（图片来源： [Brock 等，2017](https://arxiv.org/abs/1708.05344)）*
...
**MetaQNN** [Baker 等，2017](https://arxiv.org/abs/1611.02167)会训练一个代理，用 [Q 学习](https://libertydream.github.io/2020/07/05/强化学习综述/#-q-学习非策略-td-控制)法有序选择 CNN 层，还用上了 ε-greedy 探索策略和经历回放。回报就是验证准确率。
...
[Real 等，2018](https://arxiv.org/abs/1802.01548) 用演化算法（EA）来搜索高性能网络结构，命名为 **AmoebaNet**。作者采用了[锦标赛选举](https://en.wikipedia.org/wiki/Tournament_selection)法，该方法每轮会从一组随机样本中选择最好的那一项并将其变异后代放回目标总体。若锦标赛规模为 $$1$$，就等价于随机选择。
...
图 12  AmoebaNet 中的两类变异（图片来源：[Real 等，2018](https://arxiv.org/abs/1802.01548)）

作者进行的实验中 EA 和 RL 在最终验证准确率上表现一样好，但 EA 在任意时刻上的性能会更好而且能找到更小的模型。这里在 NAS 中用 EA 计算开销还是很大，每个实验都是用 450 个 GPU 跑了 7 天。

**HNAS**（[Liu 等，2017](https://arxiv.org/abs/1711.00436)） 也是用演化算法（原始锦标赛选举）当作他们的搜索策略。层次结构搜索空间中，每条边都是一个操作，所以他们实验中的基因型变异就是用一个不同的操作随机替换掉一条边。替换集中包括 `none` 选项，所以它能修改、删除和添加一条边。他们通过在“平凡”样式（所有单位映射，即输入=输出）上进行大量随机变异构建初始基因型组。

#### 渐进决策过程

构建一个模型结构是个有序过程，加入的每个运算或层都会带来额外的复杂度。如果我们引导搜索模型从简单模型开始调研，逐步演化到更复杂的架构上，这很像把“课程”引进到搜索模型的学习过程中了。

渐进 NAS（Progressive NAS，**PNAS**； [Liu 等，2018](https://arxiv.org/abs/1712.00559)）将 NAS 的问题定为搜索模型的复杂度逐步增加的一个过程。相较于 RL 或 EA，PNAS 采用基于序列模型的贝叶斯优化（Sequential Model-based Bayesian Optimization，SMBO) 法作为搜索策略。PNAS 的工作模式类似 A* 搜索，因为它由易到难的搜索模型的同时还在学习一个指导搜索的代理函数。
...
要用梯度下降更新结构搜索模型，需要使选择离散运算的过程可微。这些方法一般会把结构参数和网络权重的学习打包到一块扔到模型里。更多内容见下面[“单尝试”方法](# 单尝试法：搜索+评估)一节
...
- 在小规模数据集上训练
- 训练轮次更少
- 在搜索阶段训练并评估一个砍了规模的模型。比如，一旦学完了一个单元结构，我们可以大量进行单元复制或增大过滤器数量（[Zoph 等，2018](https://arxiv.org/abs/1707.07012)）
...
受 [Net2Net](https://arxiv.org/abs/1511.05641) 转换启发， [Cai 等，2017](https://arxiv.org/abs/1707.04873) 提出了 *高效结构搜索（Efficient Architecture Search，**EAS**）*。EAS 有个 RL 代理，称为元控制器，负责预测想保留功能需要进行的网络转换由此扩大网络深度或加大层的宽度。因为网络是逐步增大的，之前验证过的网络的权重可以在后续探索中复用。有了继承来的权重，新建网络只需要些轻度训练即可。

元控制器会结合当前网络架构学习生成 *网络转换行为（network transformation action）*，用一个变长字符串表示。为了处理长度不定的架构配置，元控制器采用了双向循环网络的实现方式。几个演员网络会输出不同的转换决策：

1. *Net2WiderNet* 操作是用更宽的层进行替换，也就意味着全连接层内有更多单元，亦或卷积层里有更多的过滤器，同时保留功用性。
2. *Net2DeeperNet* 操作是插入新的一层，为保留功用性新层初始状态就是在两层间加了个单位映射

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_EAS-meta-controller.png)

*图 14  高效架构搜索（EAS）基于 RL 的元控制器一览。对架构配置编码后，其会通过两个独立演员网络输出 net2net 转换行为（图片来源：[Cai 等，2017](https://arxiv.org/abs/1707.04873)）*

受到类似的启发，高效 NAS（**ENAS**；[Pham 等，2018](https://arxiv.org/abs/1802.03268)）通过在子模型间大量共享参数对 NAS 进行加速（小 1000 倍）。ENAS 核心洞见是它观察到所有采样得到的结构图都能看作是一张大*超图（supergraph）*的*子图（sub-graph）*。所有子网络都在共享该超图的权重。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_ENAS-example.png)

*图 15  （左）4 结点循环单元构成的完整搜索空间，但只有红色连接被激活（中）举例说明左侧激活子图怎么变成子模型结构（右）RNN 控制器给中间架构生成的网络参数（图片来源：[Pham 等，2018](https://arxiv.org/abs/1802.03268)）*

ENAS 会交替训练共享权重 $$\omega$$ 和控制器 $$\theta$$:

1.  LSTM 控制器参数 $$\theta$$ 用 [REINFORCE](https://libertydream.github.io/2020/07/19/策略梯度算法/#reinforce) 训练，回报 $$R(\mathbf{m}, \omega)$$ 在验证集上计算
2.  子模型的共享参数 $$\omega$$ 用标准监督学习损失训练。注意超图里和相同结点有关的不同操作会有它们各自的参数
...
带 SMASH 给定权重的模型性能和真实验证误差间的相关性，一定程度上可以预测用于模型比较的权重。我们需要一个容量足够大的 HyperNet，因为如果 HyperNet 模型相较子模型规模而言太小的话相关性就没了。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_SMASH-error-correlation.png)

*图 17  SMASH 误差相关性（图片来源：[Brock 等，2017](https://arxiv.org/abs/1708.05344)）*

SMASH 可以看作是实现参数共享的另一种方式。[Pham 等，2018](https://arxiv.org/abs/1802.03268) 指出 SMASH 有个问题是：因为权重是张量求积得到的，HyperNet 的使用限制了 SMASH 子模型在*低阶空间（low-rank space）*的权重。相对而言，ENAS 就没这个限制。

### 单尝试法：搜索+评估

为一众子模型分别进行搜索和评估代价太高，我们已经见识了像 [Brock 等，2017](https://arxiv.org/abs/1708.05344) 或 [Pham 等，2018](https://arxiv.org/abs/1802.03268) 提出的方法，单独训练一个模型就足以评估搜索空间中的任意子模型。

**单尝试（one-shot）**结构搜索进一步发展了共享权重的思想，并将结构生成与权重参数结合的更加紧密。下列方法全都将子结构的视作某张超图的不同子图，超图里公共边的权重共享。

[Bender 等，2018](http://proceedings.mlr.press/v80/bender18a/bender18a.pdf) 构建了一个很大的过参数化网络，称为**单尝试模型（One-Shot model）**，它包括搜索空间内所有可能的操作。借助 ScheduledDropPath（丢弃率随时间增长，训练终了达到 $$r^{1/k}$$，其中 $$0 < r < 1$$ 是个超参数而 $$k$$ 是抵达路径数）和一些精巧设计（比如 ghost 批归一化，只对激活结构做 L2 正则化），这么大一个模型的训练会很稳定，可用于评估超图采样得到的任何子模型。

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_one-shot-model-architecture.png)

*图 18  单尝试模型架构。每个单元有 $$N$$ 个选择块，每个选择块最多选 2 个运算。实线边每个结构都用，虚线是可选边（图片来源：[Bender 等，2018](http://proceedings.mlr.press/v80/bender18a/bender18a.pdf)）*

一旦单尝试模型训练好了，通过清零或删掉些边随机采样些不同结构出来，喂给它进行性能评估。采样过程可以用 RL 或演化代替。

作者们观察到用单尝试模型测得的准确率和同架构微调后的准确率差异可能会非常大。他们猜单尝试模型会自动学会关注网络里 *最有用* 的操作并且变得只要操作可用就很 *依赖* 这些操作。所以清空有用操作会大幅降低模型准确率，而删掉些无关紧要的部分只会带来很小的影响——所以在用单尝试模型评估的时候我们会看到更大的得分方差。
...
有些人认为 NAS 低效的一个主要原因是将结构搜索看成是一个 *黑箱最优化* 问题，所以我们会陷入像 RL，演化，SMBO等方法泥潭之中。如果转而依靠标准梯度下降，有可能使搜索过程更高效。于是乎， [Liu 等，2019](https://arxiv.org/abs/1806.09055) 提出了微分结构搜索（Differentiable Architecture Search，**DARTS**）。DARTS 对检索超图中的每条路都加上了了一个连续松弛操作，从而能通过梯度下降一并训练结构参数与权重。

这里用有向无环图（directed acyclic graph，DAG）表示。一个单元就是一个 DAG，内含由 $$N$$ 个结点构成的拓扑有序序列，每个结点都有一个要学习的隐表示 $$x_i$$，每条边 $$(i, j)$$ 都和某个操作 $$o^{(i,j)} \in \mathcal{O}$$ 挂钩，将 $$x_j$$ 转变为 $$x_i$$：
...
为保证搜索空间连续，DARTS 将特定操作的类别选择松弛为对所有操作进行 softmax，而结构搜索的任务简化成学习一系列混合概率 $$\alpha = \{ \alpha^{(i,j)} \}$$。
...
其中 $$\alpha_{ij}$$ 是维度 $$\vert \mathcal{O} \vert$$ 的向量，内部是结点 $$i$$ 和 $$j$$ 间不同运算的权重。

因为我们既想最优化网络权重 $$w$$ 又想使结构表示 $$\alpha$$ 最优，这就面临一个双层最优化问题：
...
还有一个想法和 DARTS 类似的，随机 NAS（Stochastic NAS；[Xie 等，2019](https://arxiv.org/abs/1812.09926)）用具体的分布（具体是指离散随机变量的连续松弛；[Maddison 等，2017](https://arxiv.org/abs/1611.00712)）和再参数化技巧进行连续松弛，它的目标和 DARTS 相同，都是使离散分布可微从而可以用梯度下降做最优化。

DARTS 能大幅减少 GPU 耗时，作者做了 CNN 单元搜索实验，其中 $$N=7$$，用一块 GPU 只花了 1.5 天。但因为网络结构的连续表示，它也有高 GPU 内存消耗的问题，为了将模型塞入单 GPU 的内存中，作者选了个小 $$N$$。

为了限制 GPU 内存开销，**ProxylessNAS** ([Cai 等，2019](https://arxiv.org/abs/1812.00332)) 将 NAS 看作是在 DAG 路径上的剪枝过程，并使结构参数二值化，迫使每次在两结点间只激活一条路。边被遮蔽与否的概率会稍后通过少数二值结构采样进行学习，并用 *BinaryConnect* ([Courbariaux 等，2015](https://arxiv.org/abs/1511.00363)) 更新相应概率。 ProxylessNAS 表明 NAS 和模型压缩间关系密切，通过路径压缩它能将内存开销降低一个数量级。

接着回到图表示。在 DAG 邻接矩阵 $$G$$ 中，$$G_{ij}$$ 是结点 $$i$$ 和 $$j$$ 间的边，其值可从基本操作集 $$\vert \mathcal{O} \vert$$ 里挑，$$\mathcal{O} = \{ o_1, \dots \}$$。无论是单尝试模型还是 DARTS 和 ProxylessNAS 都把各边看作是操作的混合体 $$m_\mathcal{O}(x)$$，但略有不同。

单尝试模型中 $$m_\mathcal{O}(x)$$ 就是所有操作的和。DARTS 里则是加权和，权重通过对长为 $$\vert \mathcal{O} \vert$$ 的实值架构权重向量 $$\alpha$$ 求 softmax 得到。ProxylessNAS 把 $$\alpha$$ 的 softmax 概率变成二值门，并用二值门保证每次只激活一个操作。
...
不用 BinaryConnext 的话，REINFORCE 也能用来更新参数使回报最大化，而且也不用 RNN 元控制器。
...
除开准确率，ProxylessNAS 也将延迟作为一项重要优化指标，毕竟不同设备间推理延迟需求可能差别很大（如 CPU，GPU，移动设备等）。为使延迟可微，作者将延迟表示为网络维度的一个连续函数。混合操作的期望延迟可以写成 $$\mathbb{E}[\text{latency}] = \sum_j p_j F(o_j)$$，其中 $$F(.)$$ 是一个延迟预测模型：

![](https://raw.githubusercontent.com/LibertyDream/diy_img_host/master/img/2020-11-11_ProxylessNAS-latency.png)

*图 23  ProxylessNAS 在训练中加入微分延迟损失（图片来源：[Cai 等，2019](https://arxiv.org/abs/1812.00332)）*

### 未来远景

目前为止我们已经见识了很多有趣的通过神经架构搜索进行网络架构工程自动化的新想法，而起其中很多都已取得了很不错的效果。但很难推断 *为什么* 某些架构效果好，我们怎么能开发跨任务的通用组件而不依赖特定数据集。

又正如 [Elsken 等，2019](https://arxiv.org/abs/1808.05377)中所言：

> “……，目前为止它在为什么特定架构效果很好的问题上带来的启发很少，对独立运行得到的架构间的相似性也知之甚少。找到通用样式，理清为什么那些样式对高性能很重要，并研究下这些样式能否泛化到不同问题上，这都是值得思考的问题”

同时，只关心验证准确率可能不太够了（[Cai 等，2019](https://arxiv.org/abs/1812.00332)）。像日常使用的移动电话这样的设备一般内存、算力都很有限。在 AI 应用日渐加深对我们日常生活影响的同时，不可避免的也在加强设备相关性。

另一个有趣的研究是给 NAS 考虑下无标注数据和自监督学习。有标注数据规模总是有限的，同时很难说数据集是否存在偏见或者严重背离真实数据分布。

[Liu 等，2020](https://arxiv.org/abs/2003.12056) 尝试回答这样一个问题，“*我们能不能不用人工注释标签找到高质量神经架构？*”，并提出了一套新的名为*无监督神经架构搜索*（*Unsupervised Neural Architecture Search*，**UnNAS**）的结构。架构效果需要在搜索阶段以无监督的方式进行估算。论文中用三个无监督假托任务（pretext task）做了实验：图像旋转预测，上色和拼图游戏。

作者在系列 UnNAS 实验中发现：

1. 相同数据集上有监督准确率和假托准确率的相关系数很高。通常相关系数大于 0.8，不管数据集、搜索空间和假托任务
2. 跨数据集上的有监督准确率和假托准确率间的高相关性
3. 更好的假托准确率意味着更高的有监督准确率
4. UnNAS 架构的性能可以比肩有监督的对照组，尽管没有更好

一个猜测是架构效果和图像统计信息有关。因为 CIFAR-10 和 ImageNet 都是自然图像，可比较且结果可迁移。UnNAS 可能无形中在搜索阶段引入了更多的无标注数据，更好的捕捉到了图像统计信息。

超参数搜索是 ML 社区永恒的话题，同时 NAS 实现了架构工程的自动化。我们可以逐步尝试将那些需要大量人工的 ML 流程自动化。更进一步讲，有没有可能自动发现 ML 算法呢？**AutoML-Zero** ([Real 等，2020](https://arxiv.org/abs/2003.03384)) 对此作了研究，通过陈化演进算法（aging evolutionary algorithms），只需很少的约束，像只用建块这样的简单数学运算，AutoML-Zero 可以自动搜索全部 ML 算法。

它要学的组件功能有三个，每个功能只用到了很基础的操作：
...
|       [HNAS (2017)](https://arxiv.org/abs/1711.00436)        |          层次结构          |            演化（锦标赛选拔）             |         迭代训练固定次数          |
...
|       [PNAS (2018)](https://arxiv.org/abs/1712.00559)        |   NASNet 搜索空间删减版    |    SMBO; 渐进搜索复杂度越来越高的架构     |            训练 20 轮             |
|       [ENAS (2018)](https://arxiv.org/abs/1802.03268)        | 既顺次又基于单元的搜索空间 |              RL (REINFORCE)               |      用共享参数训练一个模型       |
|       [SMASH (2017)](https://arxiv.org/abs/1708.05344)       |         记忆库表示         |                 随机搜索                  | 由 HyperNet  预测被评定架构的权重 |
| [One-Shot (2018)](http://proceedings.mlr.press/v80/bender18a.html) |   一个过参数化单尝试模型   |        随机搜索（随机消除些路径）         |          训练单尝试模型           |

| 模型名称                                                | 搜索空间        | 搜索算法                               |
| ------------------------------------------------------- | --------------- | -------------------------------------- |
| [DARTS (2019)](https://arxiv.org/abs/1806.09055)        | NASNet 搜索空间 | 梯度下降（对操作取 softmax 权重）      |
| [ProxylessNAS (2019)](https://arxiv.org/abs/1812.00332) | 树状架构        | 梯度下降（BinaryConnect）或  REINFORCE |
