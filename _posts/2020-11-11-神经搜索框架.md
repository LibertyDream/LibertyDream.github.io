...
前馈网络的记忆库表示方法是[Brock 等人，2017](https://arxiv.org/abs/1708.05344) 在 [SMASH](https://lilianweng.github.io/lil-log/2020/08/06/neural-architecture-search.html#prediction-based) 中提出的。相较于运算图，作者将神经网络视作有多个可读写的记忆块的系统。每层操作被设计为：（1）从记忆块子集中读取；（2）计算结果；（3）将结果写入另一子集。比如在序列模型中，会不断从单个记忆块中读取与覆盖。
