...
1. 大幅缩减了空间规模
2. 基于[主题](https://en.wikipedia.org/wiki/Network_motif)的架构很容易用到不同数据集上
3. 

#### 层次结构

 $$\ell=1, \dots, L$$  $$(G^{(\ell)}, \mathcal{O}^{(\ell)})$$, 

- $$\mathcal{O}^{(\ell)}$$ , $$\mathcal{O}^{(\ell)} = \{ o^{(\ell)}_1, o^{(\ell)}_2, \dots \}$$
- $$G^{(\ell)}$$  $$G_{ij}=k$$  $$o^{(\ell)}_k$$  $$i$$ and $$j$$ . $$1$$

 $$\ell=1$$  $$m$$  $$\ell$$
$$
o^{(\ell)}_m = \text{assemble}\Big( G_m^{(\ell)}, \mathcal{O}^{(\ell-1)} \Big)
$$
 $$\Big( \big\{ \{ G_m^{(\ell)} \}_{m=1}^{M_\ell} \big\}_{\ell=2}^L, \mathcal{O}^{(1)} \Big), \forall \ell=2, \dots, L$$, where $$\mathcal{O}^{(1)}$$ 
