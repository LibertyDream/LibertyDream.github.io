...
前面你已经看到了怎样使用 `pytest` 定场抽取普遍依赖，减少重复代码。当几个测试的输入有轻微差异，期望结果也略有不同的时候，定场就不再那么有用了。此时，可以将单个测试定义进行[参数化](http://doc.pytest.org/en/latest/example/parametrize.html)表示，`pytest` 会用你指定的参数创建测试变量。
...
你可以用 `@pytest.mark.parametrize()` 以不同值填充结构，显著减少测试代码量：
...
`parametrize()` 的首个参数是以逗号分隔的参数名字符串，第二个参数是单值或元组列表，表示参数取值。你也可以进一步参数化把所有测试整在一起：
...
尽管这样一来代码变短了，值得注意的是测试代码也没那么清楚了。用参数化将测试数据和测试行为分开，这样会很清楚正在测试什么。

### 持续时间报告：对抗慢测试

实现代码环境和测试代码环境间的每次切换都会有[开销](https://en.wikipedia.org/wiki/Overhead_(computing))，如果测试开始就很慢，那开销可能会带来摩擦和冲突。

前面提到过跑测试的时候，使用标识过滤掉慢测试。想要改善测试速度，那知道哪个测试能带来最大程度的改进会很有用，`pytest` 可以自动记录测试持续时间并报告最碍事的几个。

用 `pytest` 命令的 `--durations` 选项把持续时间报告加到测试结果里。`--durations` 接收一个整型值 n，报告最慢的 n 个测试。报告跟在测试结果后面：
...
持续时间报告里出现的每个测试都是待加速的“候选人”，因为他们花费时间已经超过整体平均水平了。

一些测试可能有隐藏配置开销，前面提到过带有 `django_db` 标识的测试会触发 Django 测试数据库的创建过程。持续时间报告会将配置数据库的时间算入测试时间，这有一定的误导性。
...
这是一种找出依赖特定执行顺序的测试的好方法，这些测试存在着对其他测试的**状态依赖（stateful dependency）**。但如果是从头用 `pytest` 搭建测试组件，这不太可能。这更可能发生在把测试组件移植到 `pytest` 的时候。

插件会在配置描述中打印一个种子值。可以用这个值跑测试，顺序就是你尝试解决问题时设定的顺序。

#### pytest-cov

如果想看一下测试对实现代码的覆盖度，大概率会用到 [coverage](https://coverage.readthedocs.io/) 包。[pytest-cov](https://pytest-cov.readthedocs.io/en/latest/) 集成了 coverage，可以用 `pytest --cov` 查看测试覆盖报告
...
`pytest` 对传统单元测试领域之外的测试同样适用。[行为驱动开发（BDD）](https://en.wikipedia.org/wiki/Behavior-driven_development)鼓励用大白话描述用户行为和期望，这样就能用它们来判断要不要实现给定功能。pytest-bdd 能帮你用 Gherkin 给代码写功能测试
...
pytest 提供了一套生产核心设施，借助灵活的插件体系对测试进行过滤和优化，这也进一步放大了它的价值。无论你是 `unittest` “教徒” 还是在从头开始构建项目，`pytest` 都能帮到你。本教程主要讲了：
...
最后从数据科学的角度，给出了一些可以用 `pytest` 做的通用单元测试。